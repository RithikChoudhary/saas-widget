...
const AppConnectionModal: React.FC<AppConnectionModalProps> = ({
  ...
  const testConnection = async () => {
    ...
    try {
      const response = await fetch('https://server.saasdor.com//api/real-sync/test-connection', {
        method: 'POST',
        headers: {
  ...
  const syncUsers = async () => {
    ...
    try {
      const response = await fetch(`https://server.saasdor.com:5000/api/real-sync/sync/${appId}`, {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('accessToken')}`
        }
